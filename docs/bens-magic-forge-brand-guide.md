...
* **Solid Content Backplates:** All textual lists, categories, and dense card layouts (such as `.category-section`) must be enclosed in solid, highly opaque **Iron** backplates (`var(--bg-card)` / `#1C1820`). This completely masks any underlying background card-art, preventing bleed-through and ensuring the active content stands on its own elevated block.
