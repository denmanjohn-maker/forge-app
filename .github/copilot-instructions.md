...
- **Scryfall HTTP clients** require both `User-Agent: mtg-forge/1.0` AND `Accept: application/json` on every request (Scryfall returns HTTP 400 if either is missing). Both headers are configured centrally in `Program.cs` via `AddHttpClient` lambdas — do not set them inside service constructors.
- `ProxyService` generates a printable proxy sheet PDF via QuestPDF. Card images are fetched from Scryfall's `POST /cards/collection` endpoint. Use `.FitUnproportionally()` on card images — Scryfall's 488×680 px images enforce a large natural minimum size via QuestPDF's `AspectRatio` element that overflows the 180×252 pt card slots if you use the default `Image()` call.
